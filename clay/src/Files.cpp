...
/** Load STL file */
Mesh* Files::loadSTL(std::istream& stream) const
{
  Mesh *mesh = new Mesh();
  VertexSet verticesSet; //detect already read vertices ...
  char header[80];
  int nbTrianglesFile;
  stream.read(header, 80); //header
  stream.read((char*)&nbTrianglesFile, 4); //number of triangles
...
Mesh* Files::loadPLY(std::istream& stream) const {
  Mesh *mesh = new Mesh();
  TriangleVector &triangles = mesh->getTriangles();
  VertexVector &vertices = mesh->getVertices();
  std::string line;
...
Mesh* Files::load3DS(std::istream& stream) const
{
  ...
  while(stream.good())
  {
    stream.read((char*)&chunkId,2);
    stream.read((char*)&chunkLength,4);
    switch (chunkId)
    {
    case 0x4d4d: //main3DS
      break;
    ...
    case 0x4100: //obj trimesh
      break;
    case 0x4110: //vertex
      stream.read((char*)&nbTotal, sizeof(unsigned short));
    ...
    case 0x4120: //face
      ...
      for(i=0; i<nbTotal; ++i)
      {
        stream.read((char*)&iVer1,sizeof(unsigned short));
        stream.read((char*)&iVer2,sizeof(unsigned short));
        stream.read((char*)&iVer3,sizeof(unsigned short));
        stream.read((char*)&faceFlag,sizeof(unsigned short));
        Vertex &v1 = vertices[iVer1];
        Vertex &v2 = vertices[iVer2];
        Vertex &v3 = vertices[iVer3];
        v1.addTriangle(i);
    ...
    case 0x4140: //text coord
      ...
      for(i=0; i<nbTotal; ++i)
      {
        stream.read((char*)&u,sizeof(float));
